...
<form method="POST" action="{{ route('login') }}" class="card card-md">
    @csrf
    <div class="card-body">
        <h2 class="mb-3 text-center">{{ __('tabler::auth.login') }}</h2>
        ...
        <div class="mb-2">
            <label class="form-label">
                ...
                <span class="form-label-description">
                    <a href="{{ route('password.request') }}">{{ __('tabler::auth.placeholder.forgotpassword') }}</a>
                </span>
                @endif
            </label>
            <div class="input-group input-group-flat">
                <input class="form-control" type="password" name="password"
                    placeholder="{{ __('tabler::auth.placeholder.password') }}" value="{{ old('password') }}" required
                    tabindex="2" />
        ...
        {{-- <div class="mb-2">
            <label class="form-check">
                ...
                <span class="form-check-label">{{ __('tabler::auth.rememberme') }}</span>
            </label>
        </div> --}}
        <div class="form-footer">
            <button type="submit" class="btn btn-primary w-100" tabindex="4">{{ __('tabler::auth.loginbutton') }}</button>
        </div>
    </div>
</form>
...
<div class="text-center text-muted mt">
    <a href="{{ route('register') }}" tabindex="-1">{{ __('tabler::auth.createaccount') }}</a>
</div>
@endif
