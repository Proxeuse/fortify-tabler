...
<form method="POST" action="{{ url('/two-factor-challenge') }}" class="card card-md">
    @csrf
    <div class="card-body">
        <h2 class="mb-3 text-center">{{ __('tabler::auth.2fa.title') }}</h2>

        <div id="code" class="form-group mb-3 ">
            <label class="form-label">{{ __('tabler::auth.2fa.code') }}</label>
            <div>
                <input type="text" name="code" autofocus autocomplete="one-time-code" class="form-control">
                <small class="form-hint">{{ __('tabler::auth.2fa.codehelp') }}</small>
            </div>
        </div>

        <div id="emergency" class="form-group mb-3 d-none">
            <label class="form-label">{{ __('tabler::auth.2fa.emergency') }}</label>
            <div>
                <input type="text" name="recovery_code" autofocus autocomplete="one-time-code" class="form-control">
                <small class="form-hint">{{ __('tabler::auth.2fa.emergencycodehelp') }}</small>
            </div>
        </div>

        <div class="form-footer">
            <button type="submit" class="btn btn-primary w-100" tabindex="4">{{ __('tabler::auth.2fa.submit') }}</button>
        </div>
    </div>
</form>

<div class="text-center text-muted mt">
    <a href="#" id="switcher" onclick="switcherFunction();" data-id="logincode" tabindex="-1">{{ __('tabler::auth.2fa.loginrecovery') }}</a>
</div>
...
function switcherFunction(){
    ...
    if(current == "logincode"){
        ...
        switcher.text("{{ __('tabler::auth.2fa.logincode') }}");
        // hide 2fa code input
        ...
        // change text of switcher
        switcher.text("{{ __('tabler::auth.2fa.loginrecovery') }}");
        // show 2fa code input
